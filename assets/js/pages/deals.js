...
function profileName(id) { if (!id) return '—'; const p = state.profiles.find((item) => item.id === id); return p ? p.full_name : String(id).slice(0, 8); }
function loadQueueCss() { if (document.querySelector('link[href="./assets/css/deal-queues.css"]')) return; const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = './assets/css/deal-queues.css'; document.head.appendChild(link); }

function isMortgageDeal(deal) {
  const d = deal.deal_json || {}; const payments = d.payments || []; const certificates = d.certificates || []; const settlements = d.settlements || [];
...
function needsLawyer(deal) {
  const d = deal.deal_json || {}; const flags = d.flags || []; const payments = d.payments || []; const certificates = d.certificates || []; const basis = d.basis || [];
...
function hasChildren(deal) {
  const d = deal.deal_json || {}; const flags = d.flags || []; const payments = d.payments || []; const certificates = d.certificates || [];
...
function setStatus(text, type = 'info') { const el = get('pageStatus'); if (!el) return; el.className = 'status ' + type; el.textContent = text; }
function rerenderCrmView() { renderStats(); renderDeals(); }
...
function badgesForDeal(deal) { const b=[]; if(hasStop(deal))b.push('<span class="pill red">стоп</span>'); if(overdueTaskCount(deal))b.push(`<span class="pill red">просрочено ${overdueTaskCount(deal)}</span>`); if(urgentTaskCount(deal))b.push(`<span class="pill orange">срочно ${urgentTaskCount(deal)}</span>`); if(hasChildren(deal))b.push('<span class="pill red">дети</span>'); if(needsLawyer(deal))b.push('<span class="pill orange">юрист</span>'); if(isMortgageDeal(deal))b.push('<span class="pill blue">банк</span>'); if(['ready_for_deposit','ready_for_deal'].includes(deal.status))b.push('<span class="pill green">готово</span>'); return b.join('')||'<span class="pill green">обычная</span>'; }
function dealRoleHint(deal) { const role=state.profile?.role; if(role==='admin')return 'Полный контроль'; if(role==='manager')return 'Контроль группы / отдела'; if(role==='lawyer')return needsLawyer(deal)?'Юридическая проверка':'Доступ по роли'; if(role==='broker')return isMortgageDeal(deal)?'Ипотека / банк':'Доступ по роли'; return 'Моя сделка'; }
