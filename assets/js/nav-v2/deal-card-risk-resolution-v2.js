...
function canAttemptMutation(risk, deal, profile) {
  ...
  if (role === 'spn') return [deal?.created_by, deal?.seller_spn_id, deal?.buyer_spn_id].some((value) => userId === String(value || ''));
  if (role === 'lawyer' || role === 'broker') return !risk?.assigned_role || risk.assigned_role === role;
  return false;
}
...
function riskTitle(risk) { return String(risk?.title || risk?.description || risk?.risk_type || '').trim(); }
function appendAction(row, item, risk, deal, profile) {
  ...
  button.onclick = async () => {
    const dialog = buildRiskResolutionDialog({ nextState, isDemo: isDemoDeal(deal), riskTitle: riskTitle(risk) });
    ...
    try {
      const result = await rpc('nav_v2_update_risk_resolution', { p_risk_id: risk.id, p_is_resolved: nextState, p_note: note || null });
      clearActionDialogDraft(button);
      window.dispatchEvent(new CustomEvent('nav-v2:risk-resolution-updated', { detail: { riskId: risk.id, changed: result?.changed === true, isResolved: nextState } }));
