...
function handoffPanel() {
  return [...document.querySelectorAll('section.card')]
    .find((section) => clean(section.querySelector('h2')?.textContent) === 'Перед передачей юристу') || null;
}

function handoffIssues() {
  const panel = handoffPanel();
  if (!(panel instanceof HTMLElement)) return [];
  const ready = [...panel.querySelectorAll('.pill')]
    .some((pill) => clean(pill.textContent) === 'можно передавать');
  if (ready) return [];
  return [...panel.querySelectorAll('.list > .list-item')]
