...
function toggleSellerFlag(value) {
  ...
  if ((value === 'manySellers' || value === 'minorSeller') && current.has(value)) {
    current.delete('oneAdultSeller');
  }
...
const labels = {
  ...
  flags: { oneAdultSeller:'один взрослый продавец', manySellers:'несколько продавцов', minorSeller:'ребёнок-собственник', minorBuyer:'ребёнок-покупатель', minorRegistered:'зарегистрированы дети', spouse:'супруг/супруга', powerOfAttorney:'доверенность', shares:'доли', sellerBankruptcyRisk:'риск банкротства продавца', encumbrance:'арест/обременение', redevelopment:'перепланировка', sellerWillNotAttend:'продавец не будет лично' },
  payments: { cash:'собственные средства', mortgage:'ипотека', matcap:'маткапитал', certificate:'сертификат / субсидия', militaryMortgage:'военная ипотека / НИС', nominalChild:'детский номинальный счет', svoChildAccount:'деньги детей / СВО', installment:'рассрочка / остаток долга' },
...
function effectiveFlags() {
  return unique([
    ...arr('flags'),
    state.deal.objectType === 'share' ? 'shares' : '',
    state.deal.sellerHasSpouse === true ? 'spouse' : '',
    state.deal.sellerByProxy === true ? 'powerOfAttorney' : ''
  ]);
}
function hasSeller() {
...
function needsSettlements() {
  return ['deposit', 'deal'].includes(state.deal.preparationMode) || ['urgent_deposit', 'deposit_exists', 'main_deal', 'terms_discussed'].includes(state.deal.stage);
}
...
function readiness() {
  ...
  if (needsSettlements()) checks.push(check('Расчёты согласованы', state.deal.settlementsAgreed === true, 'Зафиксируйте способ и согласованность расчётов.'));
  if (needsExpenses()) checks.push(check('Расходы согласованы', state.deal.expensesAgreed === true, 'Укажите, кто оплачивает основные расходы.'));
...
function computeRoute() {
  ...
  if (needsSettlements()) route.push({ id:'settlements', title:'Расчёты', hint:'как платят', render: stepSettlements });
  if (needsExpenses()) route.push({ id:'expenses', title:'Расходы', hint:'кто что платит', render: stepExpenses });
  route.push({ id:'risks', title:'Риски', hint:'триггеры', render: stepRisks });
...
function stepStage() {
  return `<h2>На какой стадии ситуация?</h2><p class="muted">Стадия убирает лишние вопросы.</p><div class="option-grid">${option('Есть только клиент', 'Объект или вторая сторона ещё не определены.', 'set:stage:lead_only', is(state.deal.stage,'lead_only'))}${option('Объект выбран, условия не согласованы', 'Собираем основу и следующий шаг.', 'set:stage:object_chosen', is(state.deal.stage,'object_chosen'))}${option('Стороны уже договорились', 'Появятся задаток, расходы и расчёты.', 'set:stage:terms_discussed', is(state.deal.stage,'terms_discussed'))}${option('Срочно готовим задаток', 'Включаем проверки до задатка.', 'set:stage:urgent_deposit', is(state.deal.stage,'urgent_deposit'))}${option('Задаток уже был', 'Проверим, что подписано и что исправить.', 'set:stage:deposit_exists', is(state.deal.stage,'deposit_exists'))}${option('Готовим основную сделку', 'Документы, расчёты, расходы, специалисты.', 'set:stage:main_deal', is(state.deal.stage,'main_deal'))}${option('Есть проблема, нужен юрист', 'Сразу выделяем риски.', 'set:stage:legal_problem', is(state.deal.stage,'legal_problem'))}</div><div class="option-grid">${option('Продавец уже есть', 'Показывать блок продавца.', 'set:hasSeller:true', state.deal.hasSeller === true)}${option('Продавца пока нет', 'Скрыть блок продавца.', 'set:hasSeller:false', state.deal.hasSeller === false)}${option('Покупатель уже есть', 'Показывать блок покупателя.', 'set:hasBuyer:true', state.deal.hasBuyer === true)}${option('Покупателя пока нет', 'Скрыть блок покупателя.', 'set:hasBuyer:false', state.deal.hasBuyer === false)}</div>${textarea('stageComment', 'Кратко опишите ситуацию', 'Что уже согласовано, что непонятно, кто торопит, ближайшая дата?')}`;
}
function stepObjectType() {
  return `<h2>Что за объект?</h2><p class="muted">Комната и доля разделены. Для квартиры нужно уточнить: МКД или квартира на земле.</p><div class="option-grid">${option('Квартира', 'Дальше выберем МКД или квартиру на земле.', 'set:objectCategory:flat', is(state.deal.objectCategory,'flat'))}${option('Комната', 'Отдельный объект: комната, коммуналка, общежитие.', 'set:objectCategory:room', is(state.deal.objectCategory,'room'))}${option('Доля', 'Отдельный сценарий: сособственники, уведомления, нотариус.', 'set:objectCategory:share', is(state.deal.objectCategory,'share'))}${option('Дом с участком', 'Дом и земля проверяются вместе.', 'set:objectCategory:house_land', is(state.deal.objectCategory,'house_land'))}${option('Земельный участок', 'Категория, ВРИ, межевание, ограничения.', 'set:objectCategory:land', is(state.deal.objectCategory,'land'))}${option('Новостройка / ДДУ / уступка', 'Застройщик, ДДУ, уступка, эскроу.', 'set:objectCategory:new_building', is(state.deal.objectCategory,'new_building'))}${option('Коммерция', 'Юрлица, назначение, арендатор, НДС.', 'set:objectCategory:commercial', is(state.deal.objectCategory,'commercial'))}</div>${state.deal.objectCategory === 'flat' ? `<div class="card" style="box-shadow:none;margin-top:12px"><h3>Какая квартира?</h3><div class="option-grid">${option('Квартира в МКД', 'Обычная квартира в многоквартирном доме.', 'set:apartmentKind:flat_mkd', is(state.deal.apartmentKind,'flat_mkd'))}${option('Квартира на земле', 'Проверим землю, статус, вход и коммуникации.', 'set:apartmentKind:flat_ground', is(state.deal.apartmentKind,'flat_ground'))}</div></div>` : ''}`;
}
...
function stepSettlements() {
  return `<h2>Порядок расчётов</h2><p class="muted">Главный вопрос: когда продавец получает деньги и как защищаем стороны.</p><div class="option-grid">${option('Перед сделкой', '', 'toggle:settlements:beforeDeal', has('settlements','beforeDeal'))}${option('На сделке', '', 'toggle:settlements:onDeal', has('settlements','onDeal'))}${option('СБР', '', 'toggle:settlements:sbr', has('settlements','sbr'))}${option('Аккредитив', '', 'toggle:settlements:accreditive', has('settlements','accreditive'))}${option('Ячейка', '', 'toggle:settlements:cell', has('settlements','cell'))}${option('Депозит нотариуса', '', 'toggle:settlements:notaryDeposit', has('settlements','notaryDeposit'))}${option('После регистрации', 'Нужна защита продавца.', 'toggle:settlements:afterRegistration', has('settlements','afterRegistration'))}${option('СФР / сертификат после регистрации', 'Продавец ждёт часть денег.', 'toggle:settlements:pensionFund', has('settlements','pensionFund'))}</div><div class="option-grid">${option('Расчёты согласованы', 'Можно фиксировать условия.', 'set:settlementsAgreed:true', state.deal.settlementsAgreed === true)}${option('Расчёты НЕ согласованы', 'Уточнить до задатка.', 'set:settlementsAgreed:false', state.deal.settlementsAgreed === false)}</div>${textarea('settlementsComment','Комментарий по расчётам', 'Когда деньги, кто пишет расписку, как защищаем стороны?')}`;
}
function stepExpenses() {
  return `<h2>Расходы</h2><p class="muted">Убираем конфликт до подписания.</p><div class="option-grid">${option('Расходы согласованы', 'Стороны понимают, кто что платит.', 'set:expensesAgreed:true', state.deal.expensesAgreed === true)}${option('Расходы НЕ согласованы', 'Нужно согласовать до задатка.', 'set:expensesAgreed:false', state.deal.expensesAgreed === false)}</div><div class="grid"><div>${field('buyerCompanyFee','Комиссия покупателя', 'number')}</div><div>${field('sellerCompanyFee','Комиссия продавца', 'number')}</div></div><h3>Нотариус</h3><div class="option-grid">${option('Платит покупатель', '', 'set:notaryPayer:buyer', is(state.deal.notaryPayer,'buyer'))}${option('Платит продавец', '', 'set:notaryPayer:seller', is(state.deal.notaryPayer,'seller'))}${option('Пополам', '', 'set:notaryPayer:split', is(state.deal.notaryPayer,'split'))}${option('Не нужен / не ясно', '', 'set:notaryPayer:unknown', is(state.deal.notaryPayer,'unknown'))}</div>${textarea('expensesComment','Комментарий по расходам', 'Нотариус, госпошлина, банк, СБР, оценка, справки, доверенности.')}`;
}
function stepRisks() {
  const flags = effectiveFlags();
  return `<h2>Особые риски</h2><p class="muted">Отмечайте только то, что реально есть. Подробные поля появятся в нужных блоках постепенно.</p><div class="option-grid">${option('Дети / опека / детские деньги', 'Юрист до задатка.', 'toggle:flags:minorRegistered', flags.includes('minorRegistered'))}${option('Доли', 'Нотариус, уведомления, сособственники.', 'sellerFlag:shares', flags.includes('shares'))}${option('Доверенность', 'Проверить полномочия.', 'sellerFlag:powerOfAttorney', flags.includes('powerOfAttorney'))}${option('Супруг/супруга', 'Согласие или брачный режим.', 'sellerFlag:spouse', flags.includes('spouse'))}${option('Наследство', 'Проверить наследников и сроки.', 'toggle:basis:inheritLaw', has('basis','inheritLaw'))}${option('Приватизация', 'Проверить отказников.', 'toggle:basis:privat', has('basis','privat'))}${option('Решение суда', 'Юрист до задатка.', 'toggle:basis:court', has('basis','court'))}${option('Расчёт после регистрации', 'Нужна защита продавца.', 'toggle:settlements:afterRegistration', has('settlements','afterRegistration'))}${option('Арест / обременение', 'Юрист до задатка.', 'sellerFlag:encumbrance', flags.includes('encumbrance'))}${option('Риск банкротства продавца', 'Нужна проверка.', 'sellerFlag:sellerBankruptcyRisk', flags.includes('sellerBankruptcyRisk'))}</div>${textarea('riskComment','Что именно настораживает?', 'Кто против, каких документов нет, что уже обещали, что срочно?')}`;
}
function handoffText() {
  const r = readiness();
  const a = analysis();
  const lines = ['Передача заявки от СПН', '', `Что готовим: ${titleOf('preparationMode', state.deal.preparationMode)}`, `Кого сопровождаем: ${titleOf('representation', state.deal.representation)}`, `Стадия: ${titleOf('stage', state.deal.stage)}`, `Объект: ${titleOf('objectType', state.deal.objectType)}`, `Адрес: ${state.deal.address || 'не указан'}`, `Цена: ${state.deal.priceTotal || 'не указана'}`, `Задаток: ${state.deal.depositAmount || 'не указан'}`, '', `Продавец: ${hasSeller() ? 'есть/нужен блок продавца' : 'не требуется сейчас'}`, `Покупатель: ${hasBuyer() ? 'есть/нужен блок покупателя' : 'не требуется сейчас'}`, `Признаки продавца/риски: ${a.flags.map((item) => labels.flags[item] || item).join(', ') || 'не указано'}`, `Деньги: ${listText('payments')}`, `Основание права: ${listText('basis')}`, `Расчёты: ${listText('settlements')}`, `Расходы согласованы: ${yesNo(state.deal.expensesAgreed)}`, `Расчёты согласованы: ${yesNo(state.deal.settlementsAgreed)}`, '', `Готовность карточки: ${r.percent}%`, `Риск: ${a.risk}`, `Нужен юрист: ${a.needsLawyer ? 'да' : 'нет/по ситуации'}`, `Нужен брокер: ${a.needsBroker ? 'да' : 'нет/по ситуации'}`, '', `Следующий шаг: ${state.deal.clientNextStep || 'не указан'}`, `Комментарий СПН: ${state.deal.spnFinalComment || state.deal.riskComment || state.deal.stageComment || 'не указан'}`];
