...
function problemRow(item, type) {
  ...
  if (type === 'integrity') {
    ...
    return `<div class="list-item"><b>${esc(item.label || 'unknown')}</b><p class="muted">severity: ${esc(item.severity || 'warning')} · count=${esc(String(item.count_value ?? 0))}</p>${sample.length ? `<p class="muted">sample: ${esc(JSON.stringify(sample))}</p>` : ''}</div>`;
  }
...
function problemList(title, items, type) {
  ...
  return `<section class="card"><div class="section-title"><div><h2>${esc(title)}</h2><p class="muted">Нужно исправить права, RLS, policy, storage, индексы, internal RPC или ссылочную целостность.</p></div><span class="pill red">${items.length}</span></div><div class="list">${items.map((item) => problemRow(item, type)).join('')}</div></section>`;
}

function overallOk() {
  return Boolean(result?.ok && (result.storage?.ok ?? true) && (result.rls?.ok ?? true) && (result.indexes?.ok ?? true) && (result.internal?.ok ?? true) && (result.integrity?.ok ?? true));
}
...
function buildReport() {
  ...
  const integrityFindings = integrity.problems || (integrity.checks || []).filter((item) => item.has_problem);
  return [
    ...
    `data_integrity.foreign_key_count: ${integrity.foreign_key_count ?? 0}`,
    `data_integrity.check_count: ${integrity.check_count ?? 0}`,
    `data_integrity.problem_count: ${integrity.problem_count ?? 0}`,
    ...
    `data_integrity_findings: ${JSON.stringify(integrityFindings)}`,
...
function draw() {
  ...
  const integrityProblemCount = Number(integrity.problem_count || integrityFindings.length || 0);
  ...
  app.innerHTML = `<main class="nav-v2-shell">
    <section class="hero"><h1>Security hardening</h1><p>Проверка RLS, policies, views, Storage, индексов, внутренних RPC, целостности данных и прямых grants для Навигатора. CRM «Лидер» не используется.</p></section>
    <div class="status ${statusClass}">${statusText}</div>
    <section class="card">
      <div class="section-title"><div><h2>Проверка</h2><p class="muted">Доступна только owner/admin. Данные читаются через security, RLS policy, Storage, Index, Internal RPC и Data Integrity health.</p></div><span class="pill ${isAdmin() ? 'blue' : 'yellow'}">${isAdmin() ? 'owner/admin' : 'restricted'}</span></div>
    ...
    ${result ? `<section class="grid">
      ...
      <div class="card"><div class="section-title"><div><h2>Data integrity</h2><p class="muted">Orphan-ссылки в сделках, задачах, документах, участниках и профилях.</p></div><span class="pill ${integrityProblemCount ? 'red' : 'ok'}">${Number(integrity.check_count || 0)}</span></div><div class="list">${metric('FK в nav-схеме', integrity.foreign_key_count ?? 0, 'blue')}${metric('Проверок', integrity.check_count ?? 0, 'blue')}${metric('Проблем', integrity.problem_count ?? 0, integrityProblemCount ? 'red' : 'ok')}</div></div>
    ...
    ${problemList('Проблемы целостности данных', integrityFindings, 'integrity')}
...
async function runCheck() {
  ...
  draw();
  try {
    const [security, rls, storage, indexes, internal, integrity] = await Promise.all([
    ...
    result = { ...security, rls, storage, indexes, internal, integrity };
  } catch (error) {
