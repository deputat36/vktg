...
function problemList(title, items, type) {
  ...
  return `<section class="card"><div class="section-title"><div><h2>${esc(title)}</h2><p class="muted">Нужно исправить права, RLS, policy или storage-настройки.</p></div><span class="pill red">${items.length}</span></div><div class="list">${items.map((item) => problemRow(item, type)).join('')}</div></section>`;
}

function overallOk() {
  return Boolean(result?.ok && (result.storage?.ok ?? true) && (result.rls?.ok ?? true));
}
...
function draw() {
  ...
  app.innerHTML = `<main class="nav-v2-shell">
    <section class="hero"><h1>Security hardening</h1><p>Проверка RLS, policies, views, Storage и прямых grants для Навигатора. CRM «Лидер» не используется.</p></section>
    <div class="status ${statusClass}">${statusText}</div>
    <section class="card">
      <div class="section-title"><div><h2>Проверка</h2><p class="muted">Доступна только owner/admin. Данные читаются через security, RLS policy и Storage health RPC.</p></div><span class="pill ${isAdmin() ? 'blue' : 'yellow'}">${isAdmin() ? 'owner/admin' : 'restricted'}</span></div>
...
async function runCheck() {
  ...
  draw();
  try {
    const [security, rls, storage] = await Promise.all([
