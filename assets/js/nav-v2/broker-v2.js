...
function missingList(item) {
  ...
  if (!missing.length) return '<div class="status ok"><b>Минимум для первичной проверки заполнен.</b></div>';
  return `<div class="status warn"><b>Нужно уточнить:</b><ul class="broker-missing">${missing.map((value) => `<li>${esc(value)}</li>`).join('')}</ul></div>`;
}
...
function brokerCard(item) {
  ...
  return `<article class="list-item broker-card">
    ...
    <section class="broker-next-action" aria-label="Следующее действие брокера">
      <div><span class="small">Следующее действие</span><b>${esc(item.next_action || 'Проверить финансовый сценарий')}</b></div>
    ...
    <div class="broker-finance-grid">
      ...
      <div><span class="small">Требуется финансирование</span><b>${esc(fmtMoney(item.buyer_needed_amount))}</b></div>
      <div><span class="small">Первоначальный взнос</span><b>${esc(fmtMoney(item.buyer_initial_amount))}</b></div>
      <div><span class="small">Готовность денег</span><b>${esc(fmtDate(item.money_ready_date || item.buyer_ready_date))}</b></div>
    </div>

    ${(item.certificate_type || item.matcap_amount) ? `<details class="broker-support-details"><summary>Сертификаты и господдержка</summary><div class="broker-finance-grid"><div><span class="small">Сертификат</span><b>${esc(item.certificate_type || 'Не указан')}</b></div><div><span class="small">Сумма сертификата</span><b>${esc(fmtMoney(item.certificate_amount))}</b></div><div><span class="small">Срок сертификата</span><b>${esc(fmtDate(item.certificate_deadline))}</b></div><div><span class="small">Материнский капитал</span><b>${esc(fmtMoney(item.matcap_amount))}</b></div></div></details>` : ''}

    ${missingList(item)}

    <div class="broker-task-line"><div><span class="small">Брокерская задача</span><b>${esc(item.task_title || 'Задача не создана')}</b></div><div><span class="small">Менеджер</span><b>${esc(item.manager_name || 'Не назначен')}</b></div></div>
...
function draw() {
  ...
  app.innerHTML = `<main class="nav-v2-shell">
    <section class="hero broker-hero"><span class="role-home-eyebrow">Предварительная финансовая оценка</span><h1>Брокерская очередь</h1><p>Сделки, где требуется финансирование, сертификат или материнский капитал. Сначала назначьте брокера и заполните минимум для первичной проверки.</p></section>
    ...
    ${preview ? `<section class="kpi-row broker-metrics" aria-label="Сводка брокерской очереди">
      ${metric('Всего сделок', n(s.total), n(s.total) ? 'blue' : 'green')}
      ...
      ${metric('Готово к проверке', n(s.ready_for_review), 'green')}
      ${metric('Просрочены задачи', n(s.overdue_tasks), n(s.overdue_tasks) ? 'red' : 'green')}
      ${metric('С господдержкой', n(s.with_certificate) + n(s.with_matcap), 'blue')}
    </section>
    <section class="card broker-list">
      <div class="section-title"><div><h2>Очередь первичной оценки</h2><p class="muted">Сортировка учитывает назначение брокера, просрочку задачи и пробелы финансовых данных.</p></div><span class="pill ${rows.length ? 'red' : 'green'}">${rows.length}</span></div>
      <div class="tabs broker-tabs">${filterButton('attention', 'Требует внимания')}${filterButton('waiting', 'Без брокера')}${filterButton('incomplete', 'Не хватает данных')}${filterButton('overdue', 'Просрочено')}${filterButton('ready', 'Готово к проверке')}${filterButton('all', 'Все')}</div>
      <div class="list">${rows.map(brokerCard).join('') || '<div class="empty">В выбранной группе нет сделок.</div>'}</div>
    </section>
    ${dataContract()}` : `<section class="card"><p role="status" aria-live="polite">${busy ? 'Формирую брокерскую очередь…' : 'Очередь ещё не загружена.'}</p></section>`}
  </main>`;
...
async function loadPreview() {
  ...
  try {
    ...
    if (!allowed()) throw new Error('Брокерская очередь доступна брокеру, менеджеру и администратору.');
  } catch (error) {
    preview = null;
