...
function safeHref(id) {
  return `./deal-card-safe-v2.html?id=${encodeURIComponent(id)}&from=${encodeURIComponent(SOURCE)}&cache=${Date.now()}`;
}
...
function apply() {
  document.querySelectorAll('a[href*="deal-card-v2.html"]').forEach((link) => {
    ...
    const card = link.closest('article.deal-card') || link.closest('.card') || link.parentElement;
