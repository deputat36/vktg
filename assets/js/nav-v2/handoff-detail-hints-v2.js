...
function renderHints(card) {
  if (!card || !loaded || !data || card.querySelector('[data-handoff-detail-hints]')) return;
  ...
  block.innerHTML = `<h3>Что закрыть перед передачей</h3>
...
async function loadData() {
  if (!dealId) return;
  try {
    data = await rpc('nav_v2_get_deal_card', { p_deal_id: dealId }, 12000);
    loaded = true;
    apply();
  } catch (_) {
    loaded = false;
