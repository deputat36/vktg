...
function clientsText(deal) {
  ...
  if (profile.role === 'spn' && own) {
    ...
    if (sellerOwn && buyerOwn) return 'Вы ведёте коммуникацию с продавцом и покупателем.';
...
function legalText(deal) {
  const lawyer = clean(deal && deal.lawyer);
  if (lawyer) return 'Юридическая часть: ' + lawyer + '.';
  if (deal && (deal.lawyer_needed || deal.status === 'need_lawyer')) return 'Юрист нужен для оценки рисков и договоров.';
...
function applyOne(deal) {
  ...
  if (!card) return;
  let box = card.querySelector('[data-client-legal-responsibility]');
  const key = clientsText(deal) + '|' + legalText(deal);
  if (box && box.dataset.key === key) return;
  if (!box) {
    box = document.createElement('div');
    box.className = 'status';
    box.dataset.clientLegalResponsibility = 'true';
    box.style.margin = '10px 0';
    const anchor = card.querySelector('[data-responsible-spn]') || card.querySelector('.deal-head');
  ...
  title.textContent = 'Клиенты и юрист: ';
  box.appendChild(title);
  box.appendChild(document.createTextNode(clientsText(deal)));
  box.appendChild(document.createElement('br'));
  const muted = document.createElement('span');
  muted.className = 'muted';
  muted.textContent = legalText(deal);
