...
function isReworkDeal(deal) { return deal.status === 'need_info'; }
...
function needsAttention(deal) {
  return isReworkDeal(deal) || deal.risk_level === 'red' || deal.has_children || !deal.expenses_agreed || !deal.settlements_agreed || Number(deal.red_risks_count || 0) > 0;
}
...
function filterDeal(deal) {
  ...
  if (searchQuery.trim()) {
    const text = [deal.id, deal.title, deal.address, deal.object_type, deal.next_action, statusText(deal.status), isDemoDeal(deal) ? 'демо demo' : 'рабочая реальная', isReworkDeal(deal) ? 'доработка need_info нужно дозаполнить' : ''].join(' ').toLowerCase();
...
function renderKpi() {
  ...
  if (profile?.role === 'lawyer') {
    return `<div class="kpi-row"><div class="metric yellow"><span>Юридическая очередь</span><b>${lawyer}</b></div><div class="metric red"><span>Красные риски</span><b>${red}</b></div><div class="metric yellow"><span>Не хватает документов</span><b>${docs}</b></div><div class="metric yellow"><span>На доработке</span><b>${rework}</b></div><div class="metric"><span>Всего доступно</span><b>${allDeals.length}</b></div></div>`;
  }
...
function renderDealCard(deal, index) {
  ...
  const reworkNotice = isReworkDeal(deal) ? '<div class="status warn" style="margin:10px 0">Нужно дозаполнить: СПН должен исправить карточку и отправить на повторную проверку.</div>' : '';
  return `<article class="deal-card ${isDemoDeal(deal) ? 'demo-card' : ''}" ${isReworkDeal(deal) ? 'style="border-color:rgba(245,158,11,.55);background:#fffdf7"' : ''}>
...
function filterOptions() {
  if (profile?.role === 'lawyer') {
    return `<option value="lawyer">Юридическая очередь</option><option value="rework">На доработке у СПН</option><option value="red">Красные риски</option><option value="docs">Не хватает документов</option><option value="attention">На контроле</option><option value="all">Все доступные</option><option value="real">Только рабочие</option><option value="demo">Только демо</option>`;
  }
  return `<option value="all">Все сделки</option><option value="rework">На доработке у СПН</option><option value="real">Только рабочие</option><option value="demo">Только демо</option><option value="attention">На контроле</option><option value="lawyer">Юристу</option><option value="broker">Брокеру</option><option value="deposit">Готовы к задатку 80%+</option><option value="deal">Готовы к сделке 80%+</option>`;
}
...
function render() {
  ...
  document.getElementById('app').innerHTML = `<main class="nav-v2-shell">
    ...
    <div class="status ${reworkCount ? 'warn' : 'ok'}">${reworkCount ? `На доработке у СПН: ${reworkCount}. Используйте фильтр «На доработке у СПН», чтобы быстро проверить зависшие карточки.` : (profile?.role === 'lawyer' ? 'Профиль юриста: основной фокус — риски, документы, юридические стоп-факторы и комментарии по сделке.' : 'Фильтры «Юристу» и «Брокеру» учитывают статус сделки, стоп-факторы, красные риски и активность по задачам.')}</div>
    ${renderKpi()}
    <section class="card">
