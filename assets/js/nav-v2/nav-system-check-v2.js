...
function profileActiveText(profile = currentProfile) {
  if (!profile) return 'не определён';
  if (profile.is_active === true) return 'активен';
  if (profile.is_active === false) return 'выключен';
  return 'статус активности не передан этим RPC';
}

function profileActiveStatus(profile = currentProfile) {
...
function sourceProfileLine([source, profile]) {
  return `${source}: ${profile.email || 'без email'} · ${roleName(profile.role)} · ${profileActiveText(profile)}`;
}
...
function reportText() {
  const profile = currentProfile
    ? `${currentProfile.email || 'без email'} · ${roleName(currentProfile.role)} · ${profileActiveText(currentProfile)}`
...
function renderProfileCard() {
  ...
  return `<div class="list">
    ...
    <div class="list-item"><b>Статус</b><p class="muted">${esc(profileActiveText(currentProfile))}</p></div>
  </div>`;
}
...
async function checkProfile() {
  ...
  try {
    ...
    const activeStatus = profileActiveStatus(currentProfile);
    const details = `Роль: ${roleName(currentProfile.role)}. Статус: ${profileActiveText(currentProfile)}.`;
    updateCheck('Профиль и роль', activeStatus, details, currentProfile.email);
  } catch (e) {
