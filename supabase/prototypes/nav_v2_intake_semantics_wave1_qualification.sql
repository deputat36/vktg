...
-- This does not change the supported inventory, mapper, save path or production objects.

create or replace function nav_v2_private.nav_v2_intake_semantics_wave1_spec_v1()
returns jsonb
language sql
immutable
security invoker
...
as $function$
  select jsonb_build_array(
    jsonb_build_object(
      'id','spouse','owner','lawyer','risk_level','yellow',
      'blocks_deposit',false,'blocks_deal',false,
      'lawyer_request_type','determine_notary',
      'expected_decision','Определить, требуется ли согласие супруга и в какой форме.',
      'documents',jsonb_build_array(
        jsonb_build_object('type','spouse_consent_status','title','Статус согласия супруга','side','seller','owner_role','seller_spn')
      )
    ),
    jsonb_build_object(
      'id','seller_absent','owner','lawyer','risk_level','yellow',
      'blocks_deposit',true,'blocks_deal',true,
      'lawyer_request_type','check_power_of_attorney',
      'expected_decision','Определить допустимый способ участия продавца и необходимые полномочия представителя.',
      'documents',jsonb_build_array(
        jsonb_build_object('type','participation_plan','title','План участия продавца в сделке','side','seller','owner_role','seller_spn'),
        jsonb_build_object('type','power_of_attorney','title','Статус доверенности','side','seller','owner_role','seller_spn')
      )
    ),
    jsonb_build_object(
      'id','encumbrance','owner','lawyer','risk_level','red',
      'blocks_deposit',true,'blocks_deal',true,
      'lawyer_request_type','check_encumbrance',
      'expected_decision','Определить, можно ли продолжать сделку и как снять или учесть ограничение.',
      'documents',jsonb_build_array(
        jsonb_build_object('type','encumbrance_extract','title','Сведения об обременении или запрете','side','object','owner_role','lead_spn'),
        jsonb_build_object('type','release_terms','title','Условия снятия ограничения','side','object','owner_role','lead_spn')
      )
    ),
    jsonb_build_object(
      'id','inheritance','owner','lawyer','risk_level','yellow',
      'blocks_deposit',true,'blocks_deal',false,
      'lawyer_request_type','check_inheritance',
      'expected_decision','Оценить основание права, наследников и возможность безопасного задатка.',
      'documents',jsonb_build_array(
        jsonb_build_object('type','inheritance_certificate','title','Документ об основании наследования','side','seller','owner_role','seller_spn')
      )
    )
  );
$function$;

create or replace function nav_v2_private.nav_v2_qualify_intake_semantics_wave1_v1(
...
begin
  if p_adapter_result is null or jsonb_typeof(p_adapter_result) <> 'object' then
    raise exception 'Wave1 qualification requires adapter result object' using errcode = '22023';
  end if;
  if p_owner_context is null or jsonb_typeof(p_owner_context) <> 'object' then
    raise exception 'Wave1 qualification requires owner context object' using errcode = '22023';
  end if;

  begin
    v_lawyer_id := nullif(btrim(coalesce(p_owner_context->>'lawyer_id','')), '')::uuid;
  exception when invalid_text_representation then
    raise exception 'Wave1 owner context contains invalid lawyer UUID' using errcode = '22023';
  end;
  ...
  for v_rule in select value from jsonb_array_elements(v_spec) loop
    ...
    if v_risk is null then
      ...
      if v_risk->>'level' <> v_rule->>'risk_level'
         ...
         or coalesce(v_risk->'required_documents','[]'::jsonb) <> (
  ...
  return jsonb_build_object(
    ...
    'matched_candidate_rule_ids',coalesce((
      ...
      where value in ('spouse','seller_absent','encumbrance','inheritance')
    ),'[]'::jsonb),
    'qualified_rule_ids',v_qualified,
    'rule_results',v_results,
    'base_unsupported_inventory',12,
    'candidate_unsupported_after_future_integration',12-jsonb_array_length(v_qualified),
    'mandatory_stops',jsonb_build_array(
      'qualification_only_not_support',
      'base_unsupported_inventory_unchanged',
      'production_mapper_not_updated',
...
revoke all on function nav_v2_private.nav_v2_intake_semantics_wave1_spec_v1() from public, anon, authenticated;
revoke all on function nav_v2_private.nav_v2_qualify_intake_semantics_wave1_v1(jsonb,jsonb) from public, anon, authenticated;
grant execute on function nav_v2_private.nav_v2_intake_semantics_wave1_spec_v1() to service_role;
grant execute on function nav_v2_private.nav_v2_qualify_intake_semantics_wave1_v1(jsonb,jsonb) to service_role;
