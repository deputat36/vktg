...
-- Apply only after canonical/base layers, wave1 qualification/integration and wave2 qualification.
-- This is not a migration and performs no business writes.

create or replace function nav_v2_private.nav_v2_prepare_intake_legacy_save_wave2_v1(
  p_result jsonb,
...
begin
  v_base := nav_v2_private.nav_v2_prepare_intake_legacy_save_wave1_v1(
    p_result,
    p_client_request_id,
    p_server_context
  );
  v_qualification := nav_v2_private.nav_v2_qualify_intake_semantics_wave2_v1(
  ...
  where blocker <> 'effective_rule_projection_incomplete';
  if not v_effective_rule_parity then
    v_effective_blockers := v_effective_blockers || jsonb_build_array('wave2_effective_rule_projection_incomplete');
  end if;

  return v_base || jsonb_build_object(
    'integration_version',3,
    'production_ready',false,
    'wave2_qualification',v_qualification,
    'effective_supported_count',17 + jsonb_array_length(v_qualified),
    ...
    'legacy_parity',(v_base->'legacy_parity') || jsonb_build_object(
      'wave1_effective_unsupported_rule_ids',v_base_unsupported,
      'wave2_qualified_rule_ids',v_qualified,
    ...
    'gates',(v_base->'gates') || jsonb_build_object(
      'wave2_semantics',jsonb_build_object(
...
create or replace function nav_v2_private.nav_v2_build_governed_intake_write_plan_wave2_v1(
  p_result jsonb,
  p_client_request_id uuid,
  p_server_context jsonb
)
...
declare
  ...
  v_allowed boolean;
begin
  v_plan := nav_v2_private.nav_v2_build_governed_intake_write_plan_wave1_v1(
    p_result,
    p_client_request_id,
    p_server_context
  );
  v_preview := nav_v2_private.nav_v2_prepare_intake_legacy_save_wave2_v1(
    p_result,
    p_client_request_id,
    p_server_context
  );
  ...
  return v_plan || jsonb_build_object(
    'write_plan_version',3,
    'integration_version',3,
    'allowed',v_allowed,
    'production_ready',false,
    'blockers',v_effective_blockers,
    'unsupported_rule_ids',v_unsupported,
    'wave2_qualification',v_preview->'wave2_qualification',
...
create or replace function nav_v2_private.nav_v2_map_governed_intake_to_production_wave2_v1(
  p_plan jsonb
)
returns jsonb
language plpgsql
...
as $function$
declare
  v_wave_ids text[] := array['bankruptcy_risk','redevelopment','after_registration','certificate'];
  ...
  v_qualified jsonb := coalesce(p_plan #> '{wave2_qualification,qualified_rule_ids}', '[]'::jsonb);
  v_wave_rule_ids jsonb;
  v_wave_document_types text[];
  v_wave1_plan jsonb;
  v_result jsonb;
  v_wave_documents jsonb := '[]'::jsonb;
...
begin
  if p_plan is null or jsonb_typeof(p_plan) <> 'object' then
    raise exception 'Wave2 mapper requires governed plan object' using errcode = '22023';
  end if;
  if not coalesce((p_plan->>'allowed')::boolean, false) then
    raise exception 'Wave2 governed plan is blocked' using errcode = '22023';
  end if;
  if jsonb_array_length(v_effective_unsupported) > 0 then
    raise exception 'Wave2 governed plan still contains unsupported semantics' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    raise exception 'Wave2 rule is not backed by qualification evidence' using errcode = '22023';
  end if;

  select coalesce(array_agg(distinct doc->>'type' order by doc->>'type'), array[]::text[])
  into v_wave_document_types
  ...
  for v_rule_id in select value from jsonb_array_elements_text(v_wave_rule_ids) loop
    ...
    if v_rule is null or v_risk is null or v_task is null then
      raise exception 'Wave2 governed rows are incomplete' using errcode = '22023';
    end if;
    if coalesce(v_risk->>'level',v_risk->>'risk_level','') <> v_rule->>'risk_level'
      ...
      raise exception 'Wave2 risk row differs from qualified catalog contract' using errcode = '22023';
    end if;
    if v_task->>'owner_role' <> 'lawyer'
      ...
      raise exception 'Wave2 lawyer task differs from qualified catalog contract' using errcode = '22023';
    end if;
    for v_doc_spec in select value from jsonb_array_elements(v_rule->'documents') loop
      ...
      if v_doc is null
        ...
        raise exception 'Wave2 document row differs from qualified catalog contract' using errcode = '22023';
      end if;
    end loop;
  end loop;

  v_wave1_plan := jsonb_set(
    jsonb_set(
      jsonb_set(
        p_plan,
        '{tasks}',
    ...
    coalesce((
      ...
      from jsonb_array_elements(coalesce(p_plan->'documents','[]'::jsonb)) item
      where item->>'type' <> all(v_wave_document_types)
    ),'[]'::jsonb),
    true
  );
  v_wave1_plan := jsonb_set(v_wave1_plan,'{unsupported_rule_ids}','[]'::jsonb,true);
  v_result := nav_v2_private.nav_v2_map_governed_intake_to_production_wave1_v1(v_wave1_plan);

  for v_doc in select value from jsonb_array_elements(coalesce(p_plan->'documents','[]'::jsonb)) item where item->>'type'=any(v_wave_document_types) loop
    v_wave_documents := v_wave_documents || jsonb_build_array(jsonb_build_object(
      'side',nav_v2_private.nav_v2_map_intake_document_side_v1(v_doc->>'side'),
      'category','intake',
      'title',coalesce(nullif(v_doc->>'title',''),v_doc->>'type'),
      ...
      'is_required',true,
      'status',nav_v2_private.nav_v2_map_intake_document_status_v1(v_doc->>'status'),
      'source_hint','intake_scope:'||(v_doc->>'side'),
      'assigned_to',v_doc->>'owner_id',
      'responsible_role','spn',
      'due_date',nullif(v_doc->>'deadline','')
    ));
  end loop;
  ...
  v_deal := (v_result->'deal') || jsonb_build_object(
    ...
    'deal_summary',coalesce(v_result #> '{deal,deal_summary}','{}'::jsonb) || jsonb_build_object(
      'wave2_semantics_integration_version',1,
  ...
  return v_result || jsonb_build_object(
    ...
    'deal',v_deal,
    'documents',coalesce(v_result->'documents','[]'::jsonb)||v_wave_documents,
    ...
    'created_event',(v_result->'created_event') || jsonb_build_object(
      'event_data',coalesce(v_result #> '{created_event,event_data}','{}'::jsonb)||jsonb_build_object(
        'wave2_semantics_integration_version',1,
        'wave2_rule_ids',v_wave_rule_ids
      )
    )
  );
end;
...
revoke all on function nav_v2_private.nav_v2_prepare_intake_legacy_save_wave2_v1(jsonb,uuid,jsonb) from public, anon, authenticated;
revoke all on function nav_v2_private.nav_v2_build_governed_intake_write_plan_wave2_v1(jsonb,uuid,jsonb) from public, anon, authenticated;
revoke all on function nav_v2_private.nav_v2_map_governed_intake_to_production_wave2_v1(jsonb) from public, anon, authenticated;
grant execute on function nav_v2_private.nav_v2_prepare_intake_legacy_save_wave2_v1(jsonb,uuid,jsonb) to service_role;
grant execute on function nav_v2_private.nav_v2_build_governed_intake_write_plan_wave2_v1(jsonb,uuid,jsonb) to service_role;
grant execute on function nav_v2_private.nav_v2_map_governed_intake_to_production_wave2_v1(jsonb) to service_role;
