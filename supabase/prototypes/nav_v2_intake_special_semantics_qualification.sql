-- Repository-only qualification for the four remaining non-fact-triggered intake rules.
-- This file is not a migration and performs no business writes.

create or replace function nav_v2_private.nav_v2_intake_special_semantics_spec_v1()
returns jsonb
language sql
immutable
security invoker
...
as $function$
  select jsonb_build_array(
    jsonb_build_object(
      'id','legal_problem','trigger_kind','stage','trigger_value','legal_problem',
      'owner','lawyer','risk_level','red','blocks_deposit',true,'blocks_deal',false,
      'lawyer_request_type','assess_urgent_case',
      'expected_decision','Определить первый безопасный шаг и перечень данных, необходимых для решения.',
      'task_evidence','structured_legal_decision','documents','[]'::jsonb
    ),
    jsonb_build_object(
      'id','partner_agency','trigger_kind','representation','trigger_value','partner_agency',
      'owner','lawyer','risk_level','yellow','blocks_deposit',false,'blocks_deal',false,
      'lawyer_request_type','check_partner_deal',
      'expected_decision','Подтвердить границы ответственности и комплект документов сторон.',
      'task_evidence','structured_document_statuses',
      'documents',jsonb_build_array(
        jsonb_build_object('type','partner_responsibility_note','title','Распределение ответственности с партнёром','side','deal','owner_role','lead_spn')
      )
    ),
    jsonb_build_object(
      'id','flat_ground','trigger_kind','object_type','trigger_value','flat_ground',
      'owner','lawyer','risk_level','yellow','blocks_deposit',true,'blocks_deal',false,
      'lawyer_request_type','design_safe_structure',
      'expected_decision','Проверить связь помещения с землёй, входом и коммуникациями.',
      'task_evidence','structured_document_statuses',
      'documents',jsonb_build_array(
        jsonb_build_object('type','land_status','title','Статус земли','side','object','owner_role','lead_spn'),
        jsonb_build_object('type','object_title_basis','title','Основание права на объект','side','object','owner_role','lead_spn')
      )
    ),
    jsonb_build_object(
      'id','house_land','trigger_kind','object_type','trigger_value','house_land',
      'owner','lawyer','risk_level','yellow','blocks_deposit',false,'blocks_deal',false,
      'lawyer_request_type','check_document_package',
      'expected_decision','Подтвердить комплект и согласованность документов на дом и участок.',
      'task_evidence','structured_document_statuses',
      'documents',jsonb_build_array(
        jsonb_build_object('type','house_title_basis','title','Основание права на дом','side','object','owner_role','lead_spn'),
        jsonb_build_object('type','land_title_basis','title','Основание права на участок','side','object','owner_role','lead_spn'),
        jsonb_build_object('type','boundary_status','title','Статус границ участка','side','object','owner_role','lead_spn')
      )
    )
  );
$function$;

create or replace function nav_v2_private.nav_v2_qualify_intake_special_semantics_v1(
...
begin
  if p_adapter_result is null or jsonb_typeof(p_adapter_result) <> 'object' then
    raise exception 'Special semantics qualification requires adapter result object' using errcode='22023';
  end if;
  if p_owner_context is null or jsonb_typeof(p_owner_context) <> 'object' then
    raise exception 'Special semantics qualification requires owner context object' using errcode='22023';
  end if;

  begin
    v_lawyer_id := nullif(btrim(coalesce(p_owner_context->>'lawyer_id','')),'')::uuid;
  exception when invalid_text_representation then
    raise exception 'Special semantics owner context contains invalid lawyer UUID' using errcode='22023';
  end;
  ...
  for v_rule in select value from jsonb_array_elements(v_spec) loop
    ...
    v_gaps := '[]'::jsonb;
    v_trigger_actual := case v_rule->>'trigger_kind'
      when 'stage' then coalesce(v_draft->>'stage','')
      when 'representation' then coalesce(v_draft->>'representation','')
      when 'object_type' then coalesce(v_draft->>'objectType','')
      else ''
    end;
    if v_trigger_actual <> v_rule->>'trigger_value' then
      v_gaps := v_gaps || jsonb_build_array('trigger_contract_mismatch');
    end if;
    ...
    elsif v_task->>'owner_role' <> 'lawyer'
    ...
    if jsonb_array_length(v_rule->'documents')=0 then
      if exists(
        select 1 from jsonb_array_elements(coalesce(v_work_plan->'document_candidates','[]'::jsonb)) item
        where coalesce(item->'rule_ids','[]'::jsonb) @> jsonb_build_array(v_rule_id)
      ) then
        v_gaps := v_gaps || jsonb_build_array('unexpected_rule_document');
      end if;
    else
    ...
    if v_lawyer_id is null then v_gaps := v_gaps || jsonb_build_array('lawyer_owner_unresolved'); end if;
    if v_handoff_state not in ('ready','urgent_incomplete') then v_gaps := v_gaps || jsonb_build_array('lawyer_handoff_not_ready'); end if;
    if not v_adapter_allowed then v_gaps := v_gaps || jsonb_build_array('adapter_gate_blocked'); end if;
    if exists(
    ...
    v_results := v_results || jsonb_build_array(jsonb_build_object(
      'rule_id',v_rule_id,
      'trigger_kind',v_rule->>'trigger_kind',
      'trigger_value',v_rule->>'trigger_value',
    ...
    if v_gap_count=0 then v_qualified := v_qualified || jsonb_build_array(v_rule_id); end if;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'matched_candidate_rule_ids',coalesce((
      ...
      where value in ('legal_problem','partner_agency','flat_ground','house_land')
    ),'[]'::jsonb),
    ...
    'candidate_unsupported_after_future_integration',4-jsonb_array_length(v_qualified),
    'mandatory_stops',jsonb_build_array(
      'qualification_only_not_support',
      'effective_21_4_inventory_unchanged',
      'special_integration_not_added',
...
revoke all on function nav_v2_private.nav_v2_intake_special_semantics_spec_v1() from public, anon, authenticated;
revoke all on function nav_v2_private.nav_v2_qualify_intake_special_semantics_v1(jsonb,jsonb) from public, anon, authenticated;
grant execute on function nav_v2_private.nav_v2_intake_special_semantics_spec_v1() to service_role;
grant execute on function nav_v2_private.nav_v2_qualify_intake_special_semantics_v1(jsonb,jsonb) to service_role;
