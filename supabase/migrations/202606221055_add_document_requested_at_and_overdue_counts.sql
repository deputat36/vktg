...
set requested_at = coalesce(requested_at, checked_at, created_at)
...
begin
  ...
  if p_status not in ('needed', 'missing', 'requested', 'received', 'checked', 'problem') then
  ...
  set status = p_status,
      requested_at = case
        ...
        when p_status in ('needed', 'missing') then null
        else requested_at
      end,
      checked_by = case when p_status in ('received', 'checked', 'problem') then v_uid else checked_by end,
      checked_at = case
        when p_status in ('received', 'checked', 'problem') then now()
        when p_status in ('needed', 'missing', 'requested') then null
        else checked_at
      end
  where id = p_document_id;
...
begin
  ...
  enriched as (
    select
      ...
      case
        ...
        when coalesce(doc.problem_documents_count, 0) > 0 then 'urgent'
        when coalesce(doc.overdue_requested_documents_count, 0) > 0 then 'urgent'
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'readiness_deposit', readiness_deposit,
    'readiness_deal', readiness_deal,
    'lawyer_needed', lawyer_needed,
    'broker_needed', broker_needed,
    'has_children', has_children,
    'has_mortgage', has_mortgage,
    'has_matcap', has_matcap,
    'has_nominal_child_money', has_nominal_child_money,
    'expenses_agreed', expenses_agreed,
    'settlements_agreed', settlements_agreed,
    'next_action', next_action,
    'created_at', created_at,
    'updated_at', updated_at,
    'open_tasks_count', open_tasks_count,
    'red_risks_count', red_risks_count,
    'yellow_risks_count', yellow_risks_count,
    'missing_documents_count', missing_documents_count,
    'requested_documents_count', requested_documents_count,
    'overdue_requested_documents_count', overdue_requested_documents_count,
    'oldest_requested_document_at', oldest_requested_document_at,
    'problem_documents_count', problem_documents_count,
    'not_requested_documents_count', not_requested_documents_count,
    'buyer_spn', buyer_spn,
    'seller_spn', seller_spn,
    'manager', manager_name,
    'lawyer_queue', lawyer_queue,
    'priority_score', priority_score,
    'last_spn_rework_at', last_spn_rework_at,
    'focus_reasons', to_jsonb(focus_reasons),
    'lawyer_next_action', lawyer_next_action
  ) order by priority_score desc, updated_at desc), '[]'::jsonb)
  into v_items
  from final_items;
