...
begin
  ...
  with scoped_deals as (
    ...
    select
      ...
      case remediation_code
        when 'deal_field_points_to_non_spn' then format('Заменить неверный профиль в поле %s', target_field)
        when 'deal_spn_profile_missing' then format('Заменить отсутствующий профиль в поле %s', target_field)
        when 'deal_spn_profile_inactive' then format('Заменить неактивного СПН в поле %s', target_field)
        ...
        when 'deal_spn_missing' then format('Заполнить отсутствующее поле %s', target_field)
  ...
  select
    ...
    (select count(distinct deal_id)::integer from actionable)
  into v_all_items, v_affected_deals
  from prepared item;
  ...
  select jsonb_build_object(
    ...
    'affected_deals', v_affected_deals,
    'mutation_available', false
  )
  into v_summary
  from groups;
