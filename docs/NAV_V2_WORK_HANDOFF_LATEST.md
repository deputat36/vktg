...
- Текущий `main`: `cdf834a11b6ead3ab97bb8138d485c12f6a1eb18` — merge PR #321.
...
- PR #318 и PR #321 frontend/tests-only: schema, grants, RPC, Auth, Edge Functions и рабочие строки не менялись.
- Открытых PR после merge #321 не было на момент подготовки handoff.
...
- интеграция карточки через существующий `deal-card-recheck-alert-v2.js`;
...
Охвачены два поля рабочего списка:
...
- при отсутствии visual label runtime добавляет `aria-label` из pure policy;
- каждому полю добавляется отдельный visually hidden help и связь через `aria-describedby`;
- search/filter остаются необязательными и не получают ложный `aria-invalid`;
- существующие labels, `aria-errormessage` и client validation карточки сделки не меняются.

Интеграция:

- canonical form policy расширен с пяти до семи известных field ids;
- `applyFormAssociations(document)` вызывается из уже подключённого `deals-spn-priority-hints-v2.js` до SPN-specific gate, поэтому search/filter получают semantics у всех ролей;
- существующий deals enhancement script выпущен как `v=20260715-01`;
- новый entry module не добавлен;
- фильтрация, URL state, RPC, permissions, role modes и layout не менялись;
- form runtime по-прежнему не использует RPC, storage, collector или network.
...
### Исправления по фактическому browser evidence PR #318
...
Финальный head: `a2a2281a07b333c04d37c265c89449ed2c242664`.

PASS:

- form association semantic/static/privacy contract;
- реальная интеграция через существующий deals enhancement entry;
- JavaScript и Python syntax;
- desktop/mobile accessible names для `dealSearch` и `dealFilter`;
- field-specific accessible descriptions;
- search/filter остаются `aria-required=false`;
- весь существующий form error/recovery regression;
...
- pure policy ограничена семью известными field ids;
- visual labels связываются через `for/id`, а поля без visual label получают bounded `aria-label`;
...
- search/filter semantics применяются до SPN-specific gate и доступны всем ролям;
- новый deals entry module не добавлен;
- form runtime не использует `fetch`, RPC, Supabase transport, storage, collector или `MutationObserver`;
- layout, focus policy, filtering, URL state, mutation semantics и role-aware permissions не менялись.

## Supabase и рабочие данные

PR #318 и PR #321 не выполняли Supabase read/write и не создавали новый live baseline.
...
- deals search/filter labels PR #321;
...
`@GitHub продолжай Navigator v2 с docs/NAV_V2_WORK_HANDOFF_LATEST.md после PR #321. Не повторяй privacy measurement, keyboard focus, async feedback, screen structure, form association или deals filter labels. Начни choice-group semantics slice: fieldset/legend или bounded role=group + aria-labelledby, общий help/error context и native keyboard behaviour для checkbox/radio groups в deal card/SPN rework/lawyer cycle. Сохрани mobile disclosure, focus continuity, async feedback, role-aware permissions и mutation semantics. Без RPC, storage, collector, backend и платной Supabase branch.`
