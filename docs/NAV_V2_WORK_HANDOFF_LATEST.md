...
- Дата: 2026-07-15.
- Текущий `main`: `23c2672546a183cd0b72a0cfb570cb07d498d31e` — merge PR #298.
...
Закрыт общий цикл после выполнения главного действия:

`сохранить → подтвердить сервером → показать результат → выбрать следующий шаг`

- карточка показывает недавнее завершение задачи, проверку документа, устранение риска или переход сделки на следующий этап;
- audit event принимается только когда совпадает с текущим состоянием сущности;
- повторно открытые, обратные, no-op и старше семи дней события не выдаются за выполненный результат;
- показаны результат, автор или достоверно определимая роль, время и понятный серверный факт;
- существующая action-first модель сразу выбирает следующее действие, ответственного, срок, критерий готовности и точный раздел;
- viewer остаётся read-only, mutation controls не добавлены;
- hook использует уже загруженные `deal/tasks/documents/risks/events` без отдельного read RPC;
- прежний module budget карточки сохранён: 19 entry modules.

Новых RPC, migrations, grants и production mutations при deploy нет.

## Проверки PR #294/#296/#298
...
Для PR #298 прошли 18/18 workflow: dedicated completion evidence, полный static suite, JavaScript syntax, action focus, SPN rework/save, lawyer document cycle, BAZA, risk и совместимые operational contracts. Public desktop/mobile job завершён success и сохранил browser evidence. Review threads: 0.
...
## Production smoke после PR #298

GitHub Pages отдаёт актуальные release-маркеры:

- `deal-card-v2.html` → `nav-v2-completion-evidence.css?v=20260715-01`;
- lifecycle mapping → `deal-card-recheck-alert-v2.js?v=20260715-15`;
- lifecycle содержит `applyDealCardCompletionEvidence` после action-focus расчёта;
- pure-модель содержит task/document/risk/stage events, семидневную актуальность и повторное использование `buildDealActionFocus`;
- hook показывает серверное подтверждение и кнопку точного перехода к следующему действию;
- новый CSS содержит desktop/mobile layout и заменяет дублирующее документное подтверждение юриста.
...
Schema, grants, functions, Auth users и рабочие строки PR #298 не менял. В production ещё нет новых completion events, поэтому блок не показывает исторический успех; он появится после будущего подтверждённого действия.
...
1. Менеджерский контроль фактически подтверждённых результатов, а не только открытого backlog.
2. Для каждой строки менеджера связать подтверждённый результат с автором, временем, следующим владельцем и контрольным сроком.
3. Не показывать старые audit events как работу «сегодня»; отделить фактическое завершение от простого изменения статуса.
...
`@GitHub @Supabase продолжай Navigator v2 с docs/NAV_V2_WORK_HANDOFF_LATEST.md после PR #298. Один раз проверь ручные gates. Если они не изменились, не повторяй SPN rework, lawyer document cycle и общий completion evidence. Начни менеджерский контроль подтверждённых результатов: руководитель должен видеть не только backlog, но и что фактически завершено, кем, когда, какой серверный факт это подтверждает, кто отвечает за следующий шаг и до какого срока. Сначала переиспользуй текущие task/document/risk/status events и manager payload; не показывай старые события как работу сегодня, не меняй рабочие данные и не создавай платную Supabase branch без точного approval #282. Заверши branch → PR → CI → merge → production smoke → handoff.`
