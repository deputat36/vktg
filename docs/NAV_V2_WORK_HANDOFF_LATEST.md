...
- `main`: `318a86d47a5ef5106819eca469313dc3afe6a109` — squash merge PR #451.
...
Read-only snapshot после PR #451:
...
- более новая session другой вкладки не перезаписывается и не очищается старым refresh response;
- logout во время refresh не приводит к восстановлению session;
- fallback compare-before-write/invalidate сохраняет защиту без Web Locks.

Merge PR #444: `d55c7646dea7d5d130f1b4571ed15610e2a6395f`.

Dedicated Auth tests, static, JavaScript, desktop/mobile Chromium и public smoke — success. Authenticated smoke корректно skipped без preview secrets.
...
Canonical PostgreSQL 17 harness по `100000` synthetic answers выполняет шесть cases:
...
Закрытые PR #447 и #448 не смержены, поскольку их проверки уже вошли в canonical evidence. Параллельные источники истины не создавались.

### PR #451 — exact non-PII query-to-index mapping

Merge: `318a86d47a5ef5106819eca469313dc3afe6a109`.

Исправлена прежняя широкая трактовка consumers.

`nav_user_profiles_role_idx`:

- 24 функции с reference на `nav_user_profiles` и `role`;
...
- прямых role-first filters — 2;
- non-demo direct role-first filter — 1;
...
Static validator доказал полное совпадение category union с исходными 24 signatures и двумя answers consumers без пропусков и дублей. PII и business rows не использовались.
...
Для `nav_deal_answers_v2_deal_idx` synthetic structural/FK semantics/scan attribution и exact consumer mapping закрыты, но отсутствуют:
...
7. подготовить production-scale FK benchmark contract только как fail-closed plan без cloud execution;
8. подготовить write-amplification/storage-benefit measurement contract без production DDL;
