...
- Дата: 2026-07-15.
- Текущий `main`: `f00a7dc1f4f892de7baabf0dab44e442ceb3545b` — merge PR #294.
...
## Production smoke после PR #294

GitHub Pages отдаёт актуальные release-маркеры:

- `deal-card-v2.html` → `nav-v2-spn-rework.css?v=20260715-01`;
- lifecycle mapping → `deal-card-recheck-alert-v2.js?v=20260715-13`;
- новый hook содержит `applyDealCardSpnRework` и подтверждение повторной отправки;
...
- обе rework RPC: `authenticated=true`, `anon=false`.

Schema, grants, functions, Auth users и рабочие строки PR #294 не менял.
...
1. Юрист: единый документный цикл `нужен → запросить → получен → проверить → проблема/готово` на одном экране.
2. Сначала доказать, что существующих document RPC и card payload достаточно; новый backend не добавлять без конкретного пробела.
3. Для каждого документа показать сторону, зачем он нужен, блокирует ли задаток/сделку, ответственного, срок, статус, последнее изменение, проблему/комментарий и одно следующее действие.
4. После этого — completion evidence и автоматический следующий шаг после закрытия главного действия.
5. Затем — менеджерский контроль подтверждённых результатов, мобильное упрощение и UX-метрики.

## NEXT_WORK_QUEUE

- P1 UX — собрать документный цикл юриста поверх существующих document RPC.
- P1 UX — completion evidence/next step после закрытия главной задачи.
...
`@GitHub @Supabase продолжай Navigator v2 с docs/NAV_V2_WORK_HANDOFF_LATEST.md после PR #294. Один раз проверь ручные gates. Если они не изменились, не повторяй SPN rework и начни единый документный цикл юриста: нужен → запросить → получен → проверить → проблема/готово. Сначала переиспользуй текущий card payload и document RPC, добавь pure-модель/explicit hook/semantic regression, не меняй рабочие данные и не создавай платную Supabase branch без точного approval #282. Заверши branch → PR → CI → merge → production smoke → handoff.`
