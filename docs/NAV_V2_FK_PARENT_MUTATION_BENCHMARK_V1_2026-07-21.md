...
Harness использует PostgreSQL 17 catalog function:

`pg_stat_get_xact_numscans(index_oid)`

Она возвращает transaction-local scan count конкретного synthetic index. View `pg_stat_xact_user_indexes` в PostgreSQL 17 отсутствует и в harness не используется.
...
- transaction-local scans относятся к доступному child index;
