...
- policy `nav_v2_deals_insert` вызывает `nav_v2_private.nav_v2_is_active_user((select auth.uid()))` явно;
...
- девять вызывающих функций автоматически пересозданы с `nav_v2_private.nav_v2_my_role`;
- две RLS-политики сохранили зависимость и отображают private schema-qualified ссылку;
- stale-ссылок `public.nav_v2_my_role` в функциях и policies нет;
- `anon` и `PUBLIC` не имеют `EXECUTE`;
- `authenticated` и `service_role` имеют необходимый `EXECUTE`;
- warning Security Advisor по публичной функции исчез;
- migration: `20260710182320_nav_v2_private_my_role_helper.sql`.

Ролевой regression baseline до и после migration совпал:

- lawyer: `my_role=lawyer`, view=true, edit=false;
...
Эти helpers используются большим числом RLS policies и server-side RPC. Для них простой `revoke execute from authenticated` недопустим: политики перестанут работать. Нужен перенос в `nav_v2_private`, сохранение минимального `EXECUTE` для RLS и обновление всех schema-qualified ссылок в функциях.

Перед следующим переносом нужно проверить:

1. точный список функций и policies;
2. прямые вызовы из frontend;
3. порядок переноса `is_owner_or_admin` перед `can_view_deal`/`can_edit_deal`;
...
- trigger helper `nav_v2_guard_active_spn_manager()` перенесён в `nav_v2_private`;
- RLS helper `nav_v2_is_active_user(uuid)` перенесён в `nav_v2_private`;
- role helper `nav_v2_my_role(uuid)` перенесён в `nav_v2_private`;
...
1. Перенести `nav_v2_is_owner_or_admin(uuid)` с автоматическим обновлением caller definitions и RLS assertions.
2. После успешного role regression подготовить совместный перенос `nav_v2_can_view_deal` и `nav_v2_can_edit_deal`.
