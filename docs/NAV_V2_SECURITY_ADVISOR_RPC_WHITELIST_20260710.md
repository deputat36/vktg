...
- `nav_v2_is_active_user(uuid)` находится в `nav_v2_private`;
...
Эти helpers используются несколькими RLS policies и server-side RPC. Для них простой `revoke execute from authenticated` недопустим: политики перестанут работать. Нужен перенос в `nav_v2_private`, сохранение минимального `EXECUTE` для RLS и обновление всех schema-qualified ссылок в функциях.
...
4. порядок переноса взаимозависимых helpers;
5. regression test owner/admin/manager/spn/lawyer/broker/viewer.

## DB health-check после разделения API и helpers

`nav_v2_get_rpc_grant_health()` теперь проверяет только browser-callable группы:
...
1. Перенести взаимозависимую группу `is_owner_or_admin` → `my_role` → `can_view_deal`/`can_edit_deal` одним согласованным dependency migration.
2. Обновить schema-qualified ссылки во всех вызывающих RPC.
3. Пересоздать RLS policies с явным `nav_v2_private.*`.
4. Расширить internal lockdown health private-helper проверками.
5. Выполнить role regression test.
6. Отдельно проверить legacy `nav_*` на отсутствие использования.
