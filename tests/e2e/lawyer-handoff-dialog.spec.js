...
test('lawyer handoff dialog shows the full issue list and Escape cancels mutation', async ({ page }, testInfo) => {
  ...
  await expectNoRuntimeFailures(failures, testInfo, 'lawyer-handoff-cancel');
});
...
test('confirmed lawyer handoff keeps the existing status RPC payload', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Передать юристу' }).click();
  ...
  await expectNoRuntimeFailures(failures, testInfo, 'lawyer-handoff-success');
});

test('server error restores the handoff button for another attempt', async ({ page }, testInfo) => {
  const failures = captureRuntimeFailures(page);
  await routeRpc(page, async (route) => {
    await route.fulfill({
      status: 400,
  ...
  await trigger.click();
  await expect(page.getByRole('dialog', { name: 'Передать юристу с незакрытыми пунктами?' })).toBeVisible();
  ...
  await expectNoRuntimeFailures(unexpected, testInfo, 'lawyer-handoff-server-error');
});
...
test('ready handoff leaves the existing direct handler untouched', async ({ page }, testInfo) => {
  ...
  await expectNoRuntimeFailures(failures, testInfo, 'lawyer-handoff-ready-direct');
});
...
test('demo handoff dialog clearly identifies test data', async ({ page }, testInfo) => {
  ...
  await expect(trigger).toBeFocused();
  await expectNoRuntimeFailures(failures, testInfo, 'lawyer-handoff-demo');
});
