...
begin
  if exists (
    ...
    raise exception 'bounded task columns remain after full rehearsal rollback';
  end if;

  if to_regclass('public.nav_deal_task_mutation_events_v2') is not null then
    raise exception 'bounded mutation event table remains after full rehearsal rollback';
  end if;
  ...
  if to_regprocedure('nav_v2_private.nav_v2_task_contract_catalog()') is not null
    ...
    raise exception 'bounded task functions remain after full rehearsal rollback';
  end if;

  if to_regprocedure('public.nav_v2_get_deal_card_lite(uuid)') is null then
    raise exception 'explicit lite DTO baseline is missing after bounded overlay rollback';
  end if;
...
select 'Navigator v2 bounded rehearsal bundle full rollback assertions passed' as result;
