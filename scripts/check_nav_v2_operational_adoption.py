...
def main() -> int:
    ...
    for forbidden in (
        ...
        if forbidden in lowered_sql:
            errors.append(f"operational adoption migration must remain read-only: {forbidden}")

    page = PAGE.read_text(encoding="utf-8")
    require(page, (
        "Content-Security-Policy",
        "aria-live=\"polite\"",
        "assets/js/nav-v2/operational-adoption-v2.js?v=20260713-01",
    ...
    print(
        "Navigator v2 operational adoption passed: read-only RPC, role routes, "
        "period filters, result states and registry/Advisor alignment checked"
    )
    return 0
